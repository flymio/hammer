...
DEFAULT_SIZE = 16 # Default size of a new LRUCache object, if no 'size' argument is given.
...
class LRUCache(object):
    ...
    def __contains__(self, key):
        retval = self.__dict.has_key(key)
        if retval:
            self.hit_count += 1
        else:
            self.miss_count += 1
    ...
    def __setitem__(self, key, obj):
        if self.__dict.has_key(key):
            ...
            while len(self.__heap) >= self.size:
                ...
                self._handle_delete(lru.key, lru.obj)
    ...
    def _handle_delete(self, key, value):
        """ Implement this method to be notified when an obj is removed due to the lru being full. """
        for handler in self.__handlers:
            handler(key, value)

    def register_delete_handler(self, f):
        """ Register a handler to handle when an item gets booted from a cache
        :param f: function(key, value) to handle the automatic deletes
        """
        if f not in self.__handlers:
            self.__handlers.append(f)

    def deregister_delete_handler(self, f):
    ...
    def get(self, key, default=None):
        ...
        :param key: key
        :param default: value to return if key not in cache
        """
        if key in self:
            return self[key]
        return default
